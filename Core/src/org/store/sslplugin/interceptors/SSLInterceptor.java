package org.store.sslplugin.interceptors;
...
import org.store.sslplugin.util.RequestUtil;
...
public class SSLInterceptor extends AbstractInterceptor {
    ...
    public String intercept(ActionInvocation invocation) throws Exception {
        ...
        if ( !isUseAnnotations() || action.getClass().isAnnotationPresent(Secured.class) || method2.isAnnotationPresent(Secured.class) ){
            ...
            boolean activePlugin = (action instanceof BaseAction && "Y".equalsIgnoreCase(((BaseAction) action).getStoreProperty(StoreProperty.PROP_USE_SECURED_PAGES,StoreProperty.PROP_DEFAULT_USE_SECURED_PAGES)));

            if (activePlugin && (HTTP_GET.equals(method) || HTTP_POST.equals(method)) && SCHEME_HTTP.equals(scheme)) {
        ...
        else if ( !isUseAnnotations() || action.getClass().isAnnotationPresent(Unsecured.class) || method2.isAnnotationPresent(Unsecured.class) ){

            boolean activePlugin = (action instanceof BaseAction && "Y".equalsIgnoreCase(((BaseAction) action).getStoreProperty(StoreProperty.PROP_USE_SECURED_PAGES,StoreProperty.PROP_DEFAULT_USE_SECURED_PAGES)));

            if (activePlugin && (HTTP_GET.equals(method) || HTTP_POST.equals(method)) && SCHEME_HTTPS.equals(scheme)){
