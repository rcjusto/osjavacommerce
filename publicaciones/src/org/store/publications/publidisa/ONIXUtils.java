...
public class ONIXUtils {
    ...
    private static final long MIN_PDF_SIZE = 2048;

    public static String updateProduct(OnixProduct onixProduct, Store20Database databaseConfig, String storeCode, Provider provider, String basePath, String onixPath, boolean downloadPreview) {
        ...
        try {
            ...
            try {
                ...
                if (StringUtils.isNotEmpty(id)) {
                    product = dao.getProductOfProvider(provider, id);
                }
                if (notificationType != null && NOTIFICATION_TYPE_DELETE.equals(notificationType)) {  // producto eliminado del inventario
                    ...
                    if (productPrice != null && productPrice.containsKey(defaultCurrency)) {
                        ...
                        for (String currCode : productPrice.keySet()) {
                            ...
                            if (curr != null) {
                                product.setCostPrice(curr.getReverseRatio() * productPrice.get(currCode));
                    ...
                    dao.save(product);
                    dao.auditStock(product, "Updated by Publidisa", null);
                    ...
                    if (downloadPreview && product.getResource(Resource.TYPE_PREVIEW) == null && previews != null) {
                        for (Map<String, String> map : previews) {
                            if ("included".equalsIgnoreCase(map.get("mode"))) {
                                ...
                                FileOutputStream fout = null;
                                FileUtils.forceMkdir(new File(pathUpload));
                                String filePath = pathUpload + FilenameUtils.getName(map.get("file"));
                                ...
                                if (previewFile.exists() && previewFile.length() > MIN_PDF_SIZE) {
